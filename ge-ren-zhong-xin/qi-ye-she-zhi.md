...
数据权限用于配制安全组内成员账号可见的数据范围。针对联系人、联系人分群、数据源、联系人记录、素材、表单、营销旅程、标签、表单统计、高级分群这些数据对象设置可见范围。
...
资源指联系人及联系人分群、素材、表单、旅程、标签、表单统计记录，可以将指定部门/安全组的素材共享给其他安全组。
...
Dmartech提供开放平台，客户可使用已分配的Key和Secret来调取DMT已提供的开放接口。

调取方式可通过【API认证信息】后的“查看API说明”来了解，接口类型和限制可通过“查看API文档”来了解。

不同接口调取的频次限制不同，可点击【API调取统计】中【可用接口数/接口总数】来查看，若要增加频次数据，请单独联系售后或客户经理。

注意：调取接口前，必须添加可用的IP和域名，否则会调取失败。

![](../.gitbook/assets/image%20%28632%29.png)



数据在接口传输中，可在【字段脱敏】内配置是否明文传输，若明文，则选中“不脱敏”，若要加密传输，则使用“脱敏”中的完全脱敏或者部分脱敏。
